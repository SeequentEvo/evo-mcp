...
def calculate_statistics_by_hole(df: pd.DataFrame, grade_column: str) -> pd.DataFrame:
    ...
    hole_stats = (
        valid_df.groupby("hole_id")
        .agg({grade_column: ["min", "max", "mean", "count"], "length": "sum", "grade_length": "sum"})
        .reset_index()
    )
    ...
    hole_stats["length_weighted_mean"] = (hole_stats["accumulation"] / hole_stats["total_length"]).replace(
...
def analyze_gaps(df: pd.DataFrame) -> dict:
    ...
    # Aggregate by hole
    hole_gap_stats = (
        gaps_df.groupby("hole_id").agg({"gap_length": ["count", "sum", "min", "max", "mean"]}).reset_index()
    )
    hole_gap_stats.columns = ["hole_id", "gap_count", "total_gap_length", "min_gap", "max_gap", "mean_gap"]
...
def calculate_multi_grade_statistics(df: pd.DataFrame, grade_columns: list[str]) -> dict:
    ...
    return {"grade_statistics": results, "errors": errors if errors else None}
        ...
        "data": [
            {
                ...
                "marker": {"color": "rgb(55, 126, 184)", "line": {"color": "rgb(255, 255, 255)", "width": 1}},
                "hovertemplate": (f"{grade_column}: %{{x:.3f}}<br>Sample Count: %{{y}}<br><extra></extra>"),
                "yaxis": "y",
            },
            {
                "type": "bar",
                ...
                "width": [float(w) for w in bin_widths],
                "marker": {"color": "rgb(228, 26, 28)", "line": {"color": "rgb(255, 255, 255)", "width": 1}},
                "hovertemplate": (f"{grade_column}: %{{x:.3f}}<br>Total Length: %{{y:.2f}} m<br><extra></extra>"),
                "yaxis": "y2",
                "visible": "legendonly",
            },
        ],
        "layout": {
            "title": {"text": f"{grade_column} Distribution Histogram", "font": {"size": 16}},
            "xaxis": {"title": {"text": grade_column}, "showgrid": True, "zeroline": False},
            "yaxis": {"title": {"text": "Sample Count"}, "showgrid": True, "side": "left"},
            "yaxis2": {"title": {"text": "Total Length (m)"}, "overlaying": "y", "side": "right", "showgrid": False},
            "barmode": "overlay",
            "hovermode": "closest",
            "showlegend": True,
            "legend": {"x": 1.15, "y": 1, "xanchor": "left", "yanchor": "top"},
...
def generate_grade_violin(
    df: pd.DataFrame, grade_column: str, group_by: Optional[str] = None, max_groups: int = 50
) -> dict:
    ...
    if group_by:
            ...
            traces.append(
                {
                    ...
                    "hovertemplate": (f"{group_by}: {group}<br>{grade_column}: %{{y:.3f}}<br><extra></extra>"),
                }
            )

        layout = {
            "title": {"text": f"{grade_column} Distribution by {group_by}", "font": {"size": 16}},
            "xaxis": {"title": {"text": group_by}, "showgrid": False},
            "yaxis": {"title": {"text": grade_column}, "showgrid": True, "zeroline": False},
            "violinmode": "group",
            "showlegend": True,
            "hovermode": "closest",
        ...
        metadata = {
            ...
            "total_samples": int(len(valid_df)),
            "grade_range": {"min": float(valid_df[grade_column].min()), "max": float(valid_df[grade_column].max())},
        }
        ...
        traces = [
            {
                ...
                "hovertemplate": (f"{grade_column}: %{{y:.3f}}<br><extra></extra>"),
            }
        ]

        layout = {
            "title": {"text": f"{grade_column} Distribution", "font": {"size": 16}},
            "yaxis": {"title": {"text": grade_column}, "showgrid": True, "zeroline": False},
            "showlegend": False,
            "hovermode": "closest",
        ...
        metadata = {
            "group_by": None,
            "total_samples": int(len(valid_df)),
            "grade_range": {"min": float(grades.min()), "max": float(grades.max())},
            "statistics": {
    ...
    return {"data": traces, "layout": layout, "metadata": metadata}
...
def get_collection_info(obj_dict: dict) -> list[dict]:
    ...
    for coll in collections:
        ...
        for attr in attributes:
            attr_info = {"name": attr.get("name"), "type": "continuous" if "values" in attr else "categorical"}
