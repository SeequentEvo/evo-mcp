...
Use this skill to run a single kriging task through MCP while keeping `KrigingParameters` as the canonical model.

Internally, the compute tool resolves typed objects and typed attributes, then lets SDK conversions produce canonical object references and attribute expressions.
...
- execute kriging through the `kriging_run` MCP tool
...
4. Call the MCP tool directly with primitive IDs and typed search/method inputs:
...
- Keep `workspace_id` separate from `KrigingParameters`.
...
- Prefer direct `kriging_run` arguments over intermediate payload construction.
...
- Do not patch malformed references or fabricate attribute expressions at run time. Fix upstream inputs and rerun with direct arguments.